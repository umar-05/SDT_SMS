            ...
            <div class="bg-blue-50 p-4 rounded-lg border border-blue-100">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-bold text-gray-900">{{ auth()->user()->profile->programme ?? 'Not Set' }}</p>
                    </div>
                </div>
                <p class="mt-2 text-[10px] text-blue-400 italic">* These records can only be changed by IT Staff.</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Phone Number</label>
                <input type="text" name="phone_number" value="{{ auth()->user()->profile->phone_number ?? '' }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 sm:text-sm">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Residential Address</label>
                <textarea name="address" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 sm:text-sm">{{ auth()->user()->profile->address ?? '' }}</textarea>
            </div>
